 ...
 * 없으면 {@code LAST_ACTIVE_TODAY}다(application.yml과 {@code @DefaultValue} 모두 같은 값). 문자열은 enum 이름으로 바뀐다.
 */
@ConfigurationProperties(prefix = "app.admin.stats")
public record AdminStatsProperties(
        @DefaultValue("LAST_ACTIVE_TODAY") ActiveUserCriterion activeUserCriterion
) {
}
