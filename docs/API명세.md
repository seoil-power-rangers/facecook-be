...
- 시각: 응답의 모든 시각 필드(`...At`)는 **한국 시간(KST) 기준, 오프셋 없는 문자열**이다(예: `"2026-09-16T13:10:00"`). 클라이언트는 이 값을 KST로 해석해야 한다. 서버는 시각을 `EventTime`으로 한국 시간으로 만들어 저장하고, DB가 기본값으로 채우는 컬럼도 연결 시간대(`+09:00`)로 한국 시간이 된다(#77).
