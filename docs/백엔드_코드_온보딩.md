...
- `STATUS`(기본값): `status = active`인 계정 수. 활동 시각을 보지 않아서 사실상 **정지되지 않은 전체 계정**이다.
- `LAST_ACTIVE_TODAY`: `last_active_at`이 오늘(행사 시간대 자정부터)인 계정 수. `last_active_at`은 로그인한 요청마다 `ActivityTrackingInterceptor`가 갱신한다.
...
`AdminStatsServiceTest`(기본 `STATUS` 기준, `LAST_ACTIVE_TODAY` 기준), `AdminStatsControllerTest`(관리자 조회, 참가자 거절), `SuperAccountBootstrapTest`(빈 설정, 생성, 비밀번호 갱신·유지, 다른 역할 계정 보호), `SuperAccountControllerTest`(계정·채팅방·메시지 열람, 관리자·참가자 거절).
...
- 알려진 불안정 테스트: 로컬에서 전체를 돌릴 때 `MissionCompletionConcurrencyIntegrationTest`가 가끔 실패하고, 단독으로 다시 돌리면 통과한다.
...
5. 관리자 통계를 본다. 기본 설정(`STATUS`)이라 `activeToday`는 정지된 bob을 뺀 전체 계정 수다.
