...
| 4 | 실시간·비동기 | WebSocket·Redis·접속 기록 작성, 실행기·스케줄러는 다음 PR |
...
| `mission` | 매칭별 미션 배정·진행, 관리자 완료 처리 | 3.6 (다음 PR) |
| `push` | 웹 푸시 구독·발송 | 3.7 (다음 PR) |
...
### 3.6 미션 (`mission`) — 다음 PR에서 작성

### 3.7 푸시 (`push`) — 다음 PR에서 작성
...
요청-응답(REST) 밖에서 도는 것들을 모았다. 이 장은 채팅 부분을 먼저 채웠고, 비동기 실행기와 스케줄러 전체는 미션·푸시 장과 함께 채운다.
...
### 4.4 비동기 실행기와 스케줄러 — 미션·푸시 장과 함께 작성
...
```bash
docker exec facecook-be-mysql-1 mysql -ufacecook -plocal-dev-password facecook -e "DELETE c FROM cook c JOIN users u ON c.sender_id = u.user_id WHERE u.email IN ('alice@local.test','bob@local.test'); DELETE m FROM message m JOIN users u ON m.sender_id = u.user_id WHERE u.email IN ('alice@local.test','bob@local.test'); DELETE mi FROM match_info mi JOIN users u ON mi.user_a_id = u.user_id WHERE u.email IN ('alice@local.test','bob@local.test'); DELETE p FROM profile p JOIN users u ON p.user_id = u.user_id WHERE u.email IN ('alice@local.test','bob@local.test'); DELETE FROM users WHERE email IN ('alice@local.test','bob@local.test');"
```
