...
| 3.3~3.5 | 콕, 매칭, 채팅 | 작성 |
| 3.6~3.7 | 미션, 푸시 | 작성 |
| 3.8~3.9 | 신고, 관리자·슈퍼 계정 | 다음 PR |
| 4 | 실시간·비동기 | 작성 |
| 5~9 | 데이터, 배포·운영, 테스트, 따라 해보기, 용어집 | 작성(기능 장이 늘면 보강) |
...
| `push` | 웹 푸시 구독·발송 | 3.7 |
| `report` | 신고와 관리자 처리(정지) | 3.8 (다음 PR) |
| `admin` | 관리자 통계 | 3.9 (다음 PR) |
| `superaccount` | 슈퍼 계정 생성·전체 열람 | 3.9 (다음 PR) |
...
### 3.8 신고 (`report`) — 다음 PR에서 작성

### 3.9 관리자·슈퍼 계정 (`admin`, `superaccount`) — 다음 PR에서 작성
...
실습 데이터(이메일이 `@local.test`로 끝나는 계정과 그 콕·메시지·미션 배정·매칭·프로필)를 자식 행부터 지운다.

```bash
docker exec facecook-be-mysql-1 mysql -ufacecook -plocal-dev-password facecook -e "DELETE c FROM cook c JOIN users u ON c.sender_id = u.user_id WHERE u.email LIKE '%@local.test'; DELETE m FROM message m JOIN users u ON m.sender_id = u.user_id WHERE u.email LIKE '%@local.test'; DELETE a FROM match_mission_assignment a JOIN match_info mi ON a.match_id = mi.match_id JOIN users u ON mi.user_a_id = u.user_id WHERE u.email LIKE '%@local.test'; DELETE mi FROM match_info mi JOIN users u ON mi.user_a_id = u.user_id WHERE u.email LIKE '%@local.test'; DELETE p FROM profile p JOIN users u ON p.user_id = u.user_id WHERE u.email LIKE '%@local.test'; DELETE FROM users WHERE email LIKE '%@local.test';"
```
