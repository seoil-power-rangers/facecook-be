...
sequenceDiagram
    ...
    I2->>DB: users.last_active_at 갱신 (이 서버에서 30초에 한 번만, 그 사이엔 DB에 안 감)
    I2->>C: 
...
- **트랜잭션에 붙는 명령**: 트랜잭션 하나를 여닫을 때 실제 쿼리 말고도 `SET autocommit`(2번), `COMMIT`, 읽기 전용이면 `SET SESSION TRANSACTION READ ONLY/READ WRITE`가 붙는다. 900명 부하테스트에서 이 명령들이 DB 시간의 약 25%였다. 그래서 드라이버가 읽기 전용 전환을 DB로 보내지 않게 했다(`application.yml`의 `data-source-properties`, #135). 자동 커밋은 끄지 않았다 — 끄면 `SET autocommit`도 없어지지만, 트랜잭션 밖에서 `JdbcTemplate`으로 쓴 값이 오류 없이 버려진다(통합 테스트 8개가 실제로 그렇게 실패했다).
...
| `PARTICIPANT_LIST_CACHE_SECONDS` | 탐색 목록 보관 시간(초, #133) | 10. 0이면 보관하지 않고 매번 DB에서 읽는다 |
| `DB_POOL_MAX_SIZE`, `DB_POOL_MIN_IDLE` | 서버당 DB 연결 수 최대 / 한가할 때 유지(#129, #131) | 10 / 10. 서버 최대 대수 × 최대값이 RDS 최대 연결 수(60)를 넘지 않게 한다 |
...
- 두 번 거른다(#135).
  1. **서버 메모리**: `UserActivityService`가 사용자별로 "이 서버가 마지막으로 갱신을 보낸 시각"을 기억한다. 30초 안이면 DB에 아무것도 보내지 않는다(트랜잭션도 열지 않음). 예전에는 30초 안이어도 UPDATE와 트랜잭션 명령이 요청마다 나갔다.
  2. **DB 조건**: 보낼 때는 UPDATE 문의 `WHERE last_active_at < (지금-30초)`가 다시 거른다(`UserRepository.touchLastActiveAt`). 서버가 여러 대라 다른 서버가 방금 갱신했으면 여기서 걸린다.
- 서버마다 따로 기억하므로 사용자 한 명당 서버마다 30초에 최대 한 번 UPDATE가 나간다.
...
#### 호출 흐름 ② 탐색 목록 (N+1 막기, 10초 보관)

`GET /api/profiles`는 참가자 수백 명을 한 번에 돌려준다. 각 프로필마다 "활동 중"을 판단하려면 `users.last_active_at`이 필요하다.

```mermaid
flowchart TB
    A["ProfileService.getParticipantsExcept(me)"] --> K{"ParticipantListCache<br/>보관본이 10초 안?"}
    K -- 예 --> F["보관본에서 나만 빼고 반환<br/>(DB·트랜잭션 없음)"]
    K -- 아니오 --> B["ProfileRepository.findAllByOrderByUserIdAsc<br/>@EntityGraph(user) → profile JOIN users 한 번"]
    B --> C["ProfileActivityLookup.toResponses<br/>last_active_at이 15분 안이면 isActive=true"]
    C --> S["보관본으로 저장"] --> F
```

- **10초 보관(#133)**: 900명 부하테스트에서 이 목록 쿼리 하나가 DB 시간의 33%를 차지했다(한 번에 약 1,880행). 목록은 몇 초 사이에 거의 바뀌지 않아서 `ParticipantListCache`가 만든 응답을 10초(`app.profile.participant-list-cache-seconds`) 동안 재사용한다. 기간이 지나면 한 요청만 다시 읽고, 동시에 온 요청은 그 결과를 같이 쓴다. 프로필을 만들거나 고치면 커밋 뒤에 그 서버의 보관본을 버린다. 서버마다 따로 보관하므로 다른 서버에는 최대 10초 늦게 반영된다.
...
| `profile/service/ParticipantListCache` | 탐색 목록을 서버 메모리에 10초 보관(#133) |
...
`ProfileServiceTest`(중복 생성, 학과·사진 검증, 부분 수정), `ParticipantListCacheTest`(10초 안 재사용·DB 안 감, 만료 뒤 다시 읽기, 동시 요청은 한 번만 읽기, 커밋 뒤 버리기), `ProfileActivityLookupTest`(활동 중 판정, user가 비어 있을 때 직접 조회), `ProfilePhotoUploadServiceTest`(허용 형식, 무작위 파일 이름, Content-Type), `ProfilePhotoUrlPolicyTest`, `ProfileControllerTest`(19세 미만 거절, `?active=` 빈 값 처리, 로그인 없는 요청 거절).
