...
| 3.3~3.5 | 콕, 매칭, 채팅 | 다음 PR |
| 3.6~3.7 | 미션, 푸시 | 다음 PR |
| 3.8~3.9 | 신고, 관리자·슈퍼 계정 | 다음 PR |
| 4 | 실시간·비동기 | 다음 PR(채팅·푸시와 함께) |
...
| `cook` | 콕 보내기·받기·취소·거절, 하루 한도 | 3.3 (다음 PR) |
| `match` | 매칭 목록·상세, 읽음 처리 | 3.4 (다음 PR) |
| `chat` | 채팅 REST·WebSocket, Redis 중계, 접속 기록 | 3.5 (다음 PR) |
...
### 3.3 콕 (`cook`) — 다음 PR에서 작성

### 3.4 매칭 (`match`) — 다음 PR에서 작성

### 3.5 채팅 (`chat`) — 다음 PR에서 작성
...
## 4. 실시간·비동기 — 다음 PR에서 작성

채팅·푸시 장과 함께 쓴다. 다룰 내용: WebSocket(STOMP) 연결과 구독 허용 목록, Redis Pub/Sub 중계, 접속 기록(presence), 커밋 후 실행, `@Async` 실행기(`pushExecutor`, `missionEventExecutor`), `@Scheduled` 작업.
...
### 8.3 참가자 가입은 메일이 필요하다

인증코드는 메일로만 전달된다. 로컬에 `SMTP_*`가 없으면 메일 발송이 실패하고, `VerificationCodeService`가 방금 만든 코드를 지운 뒤 502 `EMAIL_SEND_FAILED`를 돌려준다(3.1장 흐름 ①의 "메일 실패" 분기). 참가자 흐름(콕·매칭·채팅)을 로컬에서 따라 하는 방법은 콕·매칭 장과 함께 추가한다.
